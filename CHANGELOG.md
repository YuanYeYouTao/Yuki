...
从正式 3.8.3 标签至本基线的 **26 个已合并 PR**、行为边界与迁移说明见 [3.8.4 发布说明](docs/releases/v3.8.4.md)。

- Yuki 通过主 Agent 的显式 `send_message` 决定可见发言与分条发送；最终正文不再自动投递，`NO_REPLY` 可合法收尾。
- 可选的群聊语义观察、直呼名字邀请和 SELF 自主机会进入同一主 Agent；独立连续机会模型允许长时间沉寂后的偶发机会，但不保证发言。语义参与默认关闭，真实 QQ 效果仍待验证。
- SELF 自动化使用稳定主体、原群权限、统一消息发送与持久 Work 信号等待。
- 加固内部事件 ID、回执与原任务恢复；新增订阅监控并加快 GitHub Monitor 已接收队列的出队。
- 数据库目标 `0071`、Plugin API 3.0；正式 3.8.3 包的数据库目标为 `0061`。

## 3.8.3 - 2026-09-20

本版完整合并 PR 清单见 [3.8.3 发布说明](docs/releases/v3.8.3.md)。

- 统一当前数据库目标为随包迁移 head 0061，移除过时发布校验常量和手写迁移清单；CI 提前检查版本与文档基线。
- 整理固定工具说明，删除失效自动化管理工具、旧目录与动态加载后备路径，退出麦当劳 MCP。

- 增加当前/引用文件读取：文件形式图片/视频、文本、PDF、DOCX、XLSX 进入完整 Main Agent；文档有界解析、截断说明与临时文件清理，不执行附件或宏，不新增独立问答上下文。

- Issue #21：支持用户发送/引用的短视频限量抽帧查看，时间位置与无音频边界随原生图片进入完整 Main Agent；包含下载、时长、分辨率和子进程超时限制。
- 视频采样按时长自适应，时长上限、间隔和最大帧数支持运行时配置；抽帧成功、失败、取消均清理临时视频与帧文件，不建立持久视频缓存。
- 视频下载预算独立调整为默认 200 MiB，普通图片仍为 20 MiB，处理后模型输入预算不变。

- DeepSeek V4.1 原生图片输入：完整 Main Agent 直接读取受控图片，表情包后台分类独立保留。
- 修复 DeepSeek Responses 忽略内置搜索后的能力误判；模型名使用 `deepseek-flash`。
- 删除联网路由器、消息域名/关键词选择与自动换后端重跑；`both` 固定提供可用搜索工具，Tavily 默认首轮常驻。旧混合模式拼写仅作配置映射，权限、预算及来源安全不变。
