...
async def test_group_messages_from_different_senders_share_memory_batch_key(
    ...
    async with database.sessions() as session:
        ...
        canonical_space_id = await active_space_id_for(session, "2001")

    assert canonical_space_id is not None
    assert [row.conversation_key for row in rows] == [
        f"space:{canonical_space_id}",
        f"space:{canonical_space_id}",
    ]
...
async def test_ready_batch_waits_for_twelve_events_and_never_mixes_conversations(
    ...
    assert {job.conversation_key for job in claimed} == {f"space:{canonical_space_id}"}
...
def test_eligibility_author_kind_domain_matches_sql() -> None:
    ...
    duplicate = replace(
        _event(event_id=5, sender_user_id="1001"),
        author_kind="person",
        suppression_status="duplicate",
    )
    suppressed = replace(
        _event(event_id=6, sender_user_id="1001"),
        author_kind="person",
