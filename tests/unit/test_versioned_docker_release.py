...
def test_release_identity_matches_all_version_surfaces(monkeypatch: pytest.MonkeyPatch) -> None:
    assert validate_release_identity(ROOT, "v3.8.4") == VERSION
    ...
    with pytest.raises(ReleaseValidationError, match="baseline is stale"):
        validate_release_identity(ROOT, "v3.8.4")
...
def test_english_readme_must_match_release_baseline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def stale_english_readme(path: Path, *args, **kwargs):
        ...
        if path == ROOT / "README.en.md":
            return text.replace("version=3.8.4", "version=3.8.2")
        return text
...
def test_release_identity_rejects_non_final_tags() -> None:
    for tag in ["3.8.4", "v3.5", "v3.5.3-rc1", "v03.5.3", "latest"]:
...
def test_bundle_contains_only_deployment_files_and_expected_assets(tmp_path: Path) -> None:
    ...
    assert {path.name for path in assets} == {
        "yuki-3.8.4-deploy.zip",
        "yuki-3.8.4-deploy.tar.gz",
        "docker-compose.yml",
        ".env.example",
        "Yuki-3.8.4-Upgrade.md",
        "install.sh",
        "install.ps1",
        "SHA256SUMS",
    }
    with zipfile.ZipFile(tmp_path / "yuki-3.8.4-deploy.zip") as archive:
        names = set(archive.namelist())
        shell_mode = archive.getinfo("yuki-3.8.4-deploy/install.sh").external_attr >> 16
    prefix = "yuki-3.8.4-deploy/"
    ...
    assert (tmp_path / "Yuki-3.8.4-Upgrade.md").read_bytes() == (
        ROOT / "docs/upgrade-3.8.4.md"
    ).read_bytes()
    assert (tmp_path / "Yuki-3.8.4-Upgrade.md").read_bytes() != (
        ROOT / "docs/releases/v3.8.4.md"
    ).read_bytes()
        ...
        def run(self, *arguments: str, capture: bool = False) -> str:
            ...
            if arguments[:4] == ("exec", "-T", "bot", "python"):
                if "urllib.request" in arguments[-1]:
                    return (
                        '{"status":"ok","version":"3.8.4","database":"ok",'
