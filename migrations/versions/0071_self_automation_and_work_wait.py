...
def upgrade() -> None:
    ...
    if not _has_column("automations", "creator_kind"):
    ...
    if "uq_automation_self_creation_call" not in {
        index["name"] for index in sa.inspect(op.get_bind()).get_indexes("automations")
    }:
        op.create_index(
    ...
    if not _has_table("runtime_work_waits"):
        op.create_table(
            "runtime_work_waits",
