...
已验证阈值。主题门槛不得低于背景；profile 不符/不可用时先用精确匹配，无精确匹配则
最多一个明确标为未校准的FTS候选，不接受纯语义候选或背景补位，主动 lexical 查询仍可用。
默认四条、背景最多一条补位，不是必须注入四条。旧的明确数量配置需审计后
定向调整，不会被新默认值覆盖。

3.8.2跟进部署须核对有效值，而非只核对源码默认：普通批次等待3600秒，
`MEMORY_CONSOLIDATION_MAX_OUTPUT_TOKENS=4096`。旧环境中的300/1200会覆盖默认值；
变更前备份并记录差异，保留已有更高预算。所有生成任务仍至少low，不修改人格与密钥。
自省单段引用仍最多8条、整条16条；修复应拆分段落保留有效证据，不删引用冒充成功。
...
自省的输出 schema 保留字段说明：持续自我认识放 fact/preference，单次经历放 Episode，
不能用 fact 绕过一条 Episode 上限。Episode 只围绕一个核心经历，不能因同日同群而合并
无关话题。共享人格文件不变；这是结构化任务合同，不是主聊天的新人格提示。
输入中的已有事实/偏好与历史 Episode 分别放在 `self_facts`、`existing_episodes`，保留
原文和稳定的 fact 引用供核对、去重，不将旧文风作为新写入标准。事件证据引用白名单
必须与实际渲染的 `events` 一致；输入预算外的事件不可引用，不能因它在数据库中存在
就把未展示的内容接纳为模型证据。上下文和历史记忆仍不具备本批新证据资格。
长度校验的定向修复附带实际数量和允许上限等数值信息，不附带 Pydantic 原始 context。
仍最多修复一次；不通过时不截断输出冒充成功。字段说明与数值限制不能保证语义质量，
必须人工核对实际提交内容。当前真实复测仍存在群 Episode 混杂话题，不代表验收通过。
