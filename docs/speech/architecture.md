...
当前出站语音链路是本地合成：`Main Agent send_message.voice → SocialService →
VoiceDeliveryService → SpeechService → Unix Domain Socket → Genie-TTS Worker →
本地 GPT-SoVITS ONNX → WAV → SocialService → OneBot record`。
...
发送由显式工具调用驱动，不存在单独的 `send_voice` 主 Agent 工具。
人物偏好、默认模式配置的实际消费边界见[语音发送与人物偏好](planner-integration.md)。
