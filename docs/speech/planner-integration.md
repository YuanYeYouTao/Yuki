...
主 Agent 自行决定是否发送语音，后端核验目标权限、人物 `text_only` 偏好、声线和
...
未保存偏好时使用 `SPEECH_DEFAULT_MODE`：text 对应 text_only，optional 对应 auto，
voice/text_and_voice 对应 prefer_voice。不能据模式名称假定每次语音自动附送文字。
