...
本合同按用户最新决定覆盖原“后台 Flash 关闭思考”的配置。应用及数据库版本遵循[当前发布基线](../../README.md)，本合同不单独固定版本。
...
激活需重启的配置时，重新构造并校验 Settings，避免复制其已缓存的 model_runtime 等
分域视图而继续使用旧参数。重建失败在创建客户端前中止，错误消息不展示原始配置映射。
...
当前实际请求检查点为 94/96；一次真实 attribution 请求确认执行层发送 low，
六条全部评估且使用结果符合冻结预期，不代表全部模型任务的效果通过。未部署。整体停止点见
