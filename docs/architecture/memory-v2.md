...
该 claim 保护不等于整个模型调用与会话 generation 绑定：领取后的模型资料已经进入内存，
提交阶段核验来源人物和 claim，尚未复核会话 generation。忘记目标人物会删除其队列和
来源，使迟到结果无法提交；只重置会话或忘记同群其他人、而目标来源仍在时，已领取评估
仍可能提交。这是领取后失效处理的现存边界，不能把本次调度与事务修复描述为完整隐私取消。
