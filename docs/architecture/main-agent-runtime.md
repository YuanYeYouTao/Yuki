...
模型调用统计是观测数据，不是执行预算或副作用回执。写统计遇到数据库异常时保留
Provider 已返回的结果；失败请求的原始异常也不能被统计异常覆盖。不盲目重试未知
提交的统计 INSERT，日志 `model_invocation_record_failed` 标记 coverage_incomplete 与
本进程写入报错次数；提交结果可能未知，不把报错次数当作确定缺失数。该区间数据库
聚合只代表已落盘样本，不能宣称完整命中率。
预算、授权、请求检查点和工具回执仍按原持久化合同失败关闭。

尚未接纳工作的普通聊天也复用 RuntimeFailure 分类，分别提示数据库繁忙、内部错误、
Provider 不可用、超时或配置错误，不再把任意异常称作 AI 服务不可用。没有持久
恢复所有者时不自动重放请求；有工作时继续由既有 supervisor 退避恢复。
