...
定向证据：`tests/unit/test_runtime_work.py` 23 项，覆盖竞争租约、晚回执、输入消费、
...
CI 收集上限从 800 增至 823，用于上述新增运行时回归，未改为本地全量运行。
...
独立新工作通过同一 accept 工具登记为 queued，保留原等待任务的目标、输出类型和预算；后台按指定 work_id 激活，避免误接管同一人的另一个任务。
