...
- 一个子执行回执由唯一消费者关联父工作并唤醒。现有 continuation worker 迁移为向统一调度入口投递，不与旧主入口双跑；无 Work 的旧任务保留兼容路径直到排空。
...
| Sandbox completion | 投递父 Work 或旧兼容续跑 | 回执仅证明执行结果，不是新用户授权 |
...
| 包 | 交付内容 | 完成门槛 |
|---|---|---|
| R1 合同与状态基础 | 全入口调用图、类型、数据库增量迁移、Work/Input/Activation 仓库、状态转换与租约 | 状态归属清楚；旧路径保持可用；迁移、幂等、租约竞争的定向验证通过 |
...
| R4 全入口与恢复 | 自动化/插件/自主回复/沙箱续跑收拢、未知发送状态、重启接管 | 不重复模型唤醒、不重复执行/发送；委托边界不扩大；旧回执排空 |
...
| A15 | 插件/自动化/自主回复/旧子任务兼容 | 主声明一致；父步骤状态正确；无权限扩张、无双 worker 竞争 |
