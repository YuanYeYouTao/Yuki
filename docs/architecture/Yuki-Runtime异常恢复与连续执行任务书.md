...
状态：已实施、通过 CI 并部署。实际行为、验证和部署状态见 [异常恢复交付记录](../operations/runtime-recovery-2026-09-13.md)。
...
发送部分已按现行主 Agent 合同修订：不保留任务累计消息配额或每目标时间窗口；历史事故中
的 16 条限制仅是故障背景，不是新实现要求。实际部署状态仍须独立核对。
...
- **交付器**：按交付意图、授权和持久回执发送；不追加消息频率配额，不修改执行预算，不重新运行 Agent 获取“再发一次”的决定。
...
| 平台拒绝或发送结果未知 | 按真实拒绝/unknown 回执处理；不以本地消息频率窗口延迟发送，不盲目重发 |
...
交付状态区分未派发、dispatching、accepted、rejected、unknown；本地账本补记独立标志。**工具调用返回 ok 不直接等同于产物已交付；只有底层可核对的传输接收回执可作为交付证据。**

- 不设置本地累计消息次数、每目标时间窗口或最终回复预留槽；主 Agent 自主调用 `send_message`，最终正文不自动发送。
- 分条按文本布局与单条长度规划，按稳定子意图逐条记录；自动分条与多次主动调用均可使用。
- 真正独立的新工作使用自己的执行预算和 journal；同一人物不等于同一任务。发送统计不能成为阻止后续交付的额度。
- 旧窗口产生的 blocked 意图仅在能证明未派发时按原身份恢复；不能重跑模型重建原交付，也不能重发 accepted/unknown。
...
错误兜底只允许从结构化事实生成一条短通知，并纳入持久投递意图与回执防重放。对外不展示原始 JSON、内部路径列表、完整工具参数、凭据或 Provider 请求正文。
...
主/子调度器共用恢复监督器，自动化和插件实现 pending 回传与原步骤恢复；统一交付意图与回执，补齐状态/故障/attempt/wakeup 字段及索引。新增 Alembic 迁移，不能编辑 0056/0057/0058 等已发布迁移；启动校验继续从随代码发布的 migration head 自动读取。
...
| 同一成员旧绘图任务已发 16 条，再提出独立试卷任务 | 不附到旧目标 journal，不继承旧任务预算；新任务可运行且不受历史发送次数阻碍。换成员能聊天不能替代此验收；正常新输入改向仍需进入原工作 |
| 同目标短时间连续发送超过 16 条及大于 16 条的分条计划 | 无本地消息窗口拒绝；每条有独立持久回执，重启只恢复明确未派发部分，unknown 不重发 |
