...
2. 自动化任务的“读取可见性”必须与“修改权限”分离。Yuki 应能查看全局任务简表，并在每个群聊轮次中收到该群当前 active 任务的极简快照；普通用户只能更新、暂停、恢复、取消和立即运行自己的任务，超级管理员可以管理全部任务。当前 `automation_list` 只查询当前发言人名下任务，已经导致 Yuki 把“我无权管理”误判成“任务不存在”，进而在无关消息轮次重复创建日记任务。
...
读能力描述应是“查看 Yuki 自动化任务的安全摘要”；写能力明确“普通用户只能管理自己的任务，超级管理员可以管理全部任务”。
