...
- 被提及者和最近发言者中最多 5 人仍可提供当前群身份元数据，但最近发言者不会自动成为
  长期记忆检索目标；
- 当前私聊或当前群在 `MAX_CONTEXT_CHARACTERS`（默认 `131072`）总预算内的连续历史，其中 Actor 元数据受 `CONTEXT_METADATA_BUDGET_RATIO` 限制；
...
| `MAX_CONTEXT_CHARACTERS` | `131072`（字符，不是 token） |
| `CONTEXT_METADATA_BUDGET_RATIO` | `0.04`（约 5k 字符 Actor 元数据上限） |
| `HISTORY_WINDOW_LOW_WATERMARK_RATIO` | `0.67`（保留键；Prompt 选窗不再使用） |
| `CONVERSATION_ROLLUP_RAW_TAIL_EVENTS` | `128` |
| `CONVERSATION_ROLLUP_RAW_TAIL_CHARACTERS` | `20480` |
| `CONVERSATION_ROLLUP_TRIGGER_EVENTS` / `STOP_EVENTS` | `384` / `0` |
| `CONVERSATION_ROLLUP_TRIGGER_CHARACTERS` / `STOP_CHARACTERS` | `81920` / `0` |
| `CONVERSATION_ROLLUP_FOREGROUND_MAX_BATCHES` | `5` |
| `CONVERSATION_ROLLUP_WORKER_MAX_BATCHES_PER_CLAIM` | `5` |
| `CONVERSATION_ROLLUP_SUMMARY_MAX_CHARACTERS` | `2400` |
