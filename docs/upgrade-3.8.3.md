...
当前是尚未发布的开发基线，目标数据库为 **0061**。应用版本推进不意味着需要新增迁移。
已有生产环境若已是 0061，本轮无需改库；本次仅更新仓库和 CI，不重建线上容器。
...
正式发布后使用对应发行包；开发部署可从确认的提交本地构建镜像，再传服务器加载。
...
麦当劳退出需同步移除部署 `mcp.json` 中的 `mcd`、连接凭据及旧工具缓存；不能只更新示例文件。保留历史审计和已结束任务。普通自动化能力的权限不会因为版本更新自动扩大。
