...
class CommandService:
    ...
    async def execute(
        ...
        actor = AdminActor(
            ...
            canonical_conversation_id=message.conversation_id,
            ingress_presence_id=message.presence_id,
    ...
    def _help_text() -> str:
        return (
            ...
            "自然语言也可查历史共同群友/旧群的结构记忆，支持昵称和群名；同名需澄清。\n"
            "共同群可读 Person（含私聊来源事实），不开放原始私聊/他人证据；写入权限不变。\n"
            "普通自动提取最长等待一小时，空提取正常；明确记忆修改仍即时执行。\n"
